...
async function main() {
  ...
  const donorIdentity = `sha256:${sha(fs.readFileSync(donorPath))}`;
  ...
  for (const profile of profiles) {
    ...
    try {
      for (const viewport of matrix) {
        ...
        if (pinnedEnvironment && pinnedEnvironment !== fingerprint) throw new Error(`render environment changed within freeze at ${key}`);
        pinnedEnvironment = fingerprint;
        ...
        rows.push({ matrix_key: key, screenshot: rel, sha256: sha(buffer), environment_sha256: fingerprint, blocked_requests: network.filter(x => x.action === 'blocked').length, blocked_transports: runtimeTransports.length, runtime_errors: runtimeErrors });
        await context.close();
      }
    } finally {
      await browser.close();
  ...
  const manifest = { schema_version: '2.0', result, donor_identity: donorIdentity, environment_sha256: pinnedEnvironment, matrix_keys: rows.map(row => row.matrix_key).sort(), rows, artifacts };
