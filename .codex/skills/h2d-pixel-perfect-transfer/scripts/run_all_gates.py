...
SCHEMA_BY_TEMPLATE = {
    ...
    'token_map_template.json':'token_map.schema.json',
...
def add_generator_provenance_check(checks: list[dict[str, str]], name: str, data: Any|None, script_name: str) -> None:
    """A required report must come from its bundled generator.

    Schema plus `result` alone would accept a hand-authored passing JSON; the
    generator hash binds the report to the exact script shipped in this package.
    """
    script = ROOT/'scripts'/script_name
    if not isinstance(data, dict):
        checks.append({'name': f'generator:{name}', 'result': 'fail', 'message': 'report data missing'})
        return
    expected = sha256_file(script)
    actual = data.get('generator_sha256')
    if actual == expected:
        checks.append({'name': f'generator:{name}', 'result': 'pass', 'message': f'generated by bundled {script_name}'})
    else:
        checks.append({'name': f'generator:{name}', 'result': 'fail', 'message': f'{name} was not generated by the bundled {script_name} (generator_sha256 mismatch); regenerate it instead of authoring it'})


def add_layout_integrity_checks(checks: list[dict[str, str]], rect_data: Any|None, node_data: Any|None, tree_index: Path) -> None:
    if not isinstance(rect_data, dict) or not isinstance(node_data, dict):
        checks.append({'name':'layout_contract_integrity','result':'fail','message':'rect_targets or node_validation data missing'})
        return
    extractor = ROOT/'scripts'/'extract_rect_targets.py'
    provenance = (
        tree_index.is_file()
        and rect_data.get('source_tree_sha256') == sha256_file(tree_index)
        and rect_data.get('generator_sha256') == sha256_file(extractor)
    )
    complete_chain = rect_data.get('coverage_mode') == 'complete-ancestor-chain' and rect_data.get('includes_ancestors') is True
    rows = rect_data.get('viewports') or []
    root_map = rect_data.get('root_map') or {}
    counts_ok = bool(rows)
    targets_by_viewport: dict[int, int] = {}
    for row in rows:
        if not isinstance(row, dict):
            counts_ok = False; continue
        targets = row.get('targets') or []
        viewport = row.get('viewport')
        relations = [target.get('relation_to_scope') for target in targets if isinstance(target, dict)]
        expected_ancestors = sum(1 for relation in relations if relation == 'scope-ancestor')
        expected_layout = sum(1 for target in targets if isinstance(target, dict) and target.get('box_style'))
        ok = (
            isinstance(viewport, int)
            and row.get('root_path') == root_map.get(str(viewport))
            and row.get('target_count') == len(targets)
            and row.get('ancestor_target_count') == expected_ancestors
            and row.get('layout_target_count') == expected_layout
            and relations.count('scope-root') == 1
        )
        counts_ok = counts_ok and ok
        if isinstance(viewport, int): targets_by_viewport[viewport] = len(targets)
    measured = True
    for row in node_data.get('viewports') or []:
        if not isinstance(row, dict): measured = False; continue
        expected = targets_by_viewport.get(row.get('viewport'))
        if expected is None or row.get('checked') != expected or row.get('missing'):
            measured = False
    strict = node_data.get('lenient_box_style') is False
    injective = node_data.get('selector_map_injective') is True
    ok = provenance and complete_chain and counts_ok and measured and strict and injective
    checks.append({
        'name':'layout_contract_integrity',
        'result':'pass' if ok else 'fail',
        'message':'source-bound complete container chain measured with strict styles and an injective map' if ok else 'layout proof is stale, scope-only, count-incomplete, lenient, non-injective, or not fully measured',
    })
...
def check_output(out: Path, behavior_required_arg: str, liveness_required_arg: str='auto', accept_changed_source: bool=False) -> dict[str, Any]:
    ...
    for fn, schema, allowed in required:
        ...
        if fn == 'rect_targets.json': rect_data = d
    ...
    add_layout_integrity_checks(checks, rect_data, node_data, source_dir/'h2d_tree_index.json')
