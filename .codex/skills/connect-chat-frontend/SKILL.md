...
Важный auth-канон: пользовательский chat API вызывается напрямую из miniapp/сайта/frontend в GLAIM через source token. В HTTP source token передаётся только как заголовок `X-Source-Secret` для routes `/api/v2/sources/{source}/chat/*`. Gigma backend, ERP backend или BFF не являются частью канонической цепочки.
...
- Перед production-доступом проверь blast radius: текущий source token в GLAIM может открывать не только chat routes, но и другие `/sources/{source}/*` routes. Безопасная модель — chat-scoped source token или ограничение доступа на стороне GLAIM.
...
- Для production проверен blast radius source token; при необходимости нужен chat-scoped token.
