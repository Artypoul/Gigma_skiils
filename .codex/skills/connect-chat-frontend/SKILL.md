...
description: "Подключить miniapp, сайт, витрину или web chat напрямую к GLAIM source-chat API безопасно и по реальному контракту. Используй когда нужно сделать чат с AI-агентом через GLAIM source-chat, настроить API client, session/message/events/stop/reset flow, polling событий, историю чата, переключение тредов/source_conversation_ref, markdown-ответы, source token, source scope, client_message_id, context_snapshot или проверить, что frontend использует source token только для chat API."
---
...
6. В клиенте храни только публичное состояние:
   ...
   - список событий;
   - список UI-тредов miniapp, где каждый тред имеет свой `source_conversation_ref`;
   - локальный pending-state отправки;
   - `client_message_id` для каждого сообщения.
7. Реализуй flow:
   - открыть или создать session;
   - при открытии session отрендерить `events` из response как историю выбранного треда и сохранить `next_after_id`;
...
2. При выборе треда frontend меняет active `source_conversation_ref` и вызывает `POST /api/v2/sources/{source}/chat/session` с тем же project/app scope.
3. GLAIM возвращает active session для этого source conversation, `events` для начальной истории и `next_after_id`.
4. После этого frontend вызывает `GET /api/v2/sources/{source}/chat/sessions/{session_id}/events` с тем же scope и `after_id=next_after_id`, чтобы обновлять историю без дублей.
5. При возврате на другой тред повтори шаги 2-4 с его `source_conversation_ref`; не переиспользуй `session_id` от другого треда.

Не придумывай user-chat endpoint для списка тредов. В текущем GLAIM source-chat API нет `GET /chat/sessions`, `GET /chat/threads` или `GET /chat/history`. Список тредов принадлежит source/miniapp, а GLAIM отдаёт историю выбранного треда через `session.events` и `/events`.
...
История: при загрузке активного треда сначала используй `events` из `/chat/session`, затем догоняй новые события через `/events?after_id=<next_after_id>`. Для локального UI дедуплицируй события по `event.id`, а pending-сообщения пользователя — по `client_message_id`.
...
- Переключение тредов меняет active `source_conversation_ref`, заново открывает `/chat/session` и не переиспользует `session_id` другого треда.
...
- История обновляется через `session.events` + `/events?after_id=...`; агент не добавил несуществующий `GET /chat/threads` или `GET /chat/history`.
