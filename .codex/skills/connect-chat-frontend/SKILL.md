...
5. После этого frontend вызывает `GET /api/v2/sources/{source}/chat/sessions/{session_id}/events` с тем же scope и `after_id=next_after_id`, пока не получит пустую страницу или неизменившийся cursor; только после этого локальная история выбранного треда догружена.
6. Для live-обновления продолжай polling `/events` с последним `next_after_id`, чтобы обновлять историю без дублей.
7. При возврате на другой тред повтори шаги 3-6 с его `source_conversation_ref` и `default_agent_key`; не переиспользуй `session_id` от другого треда или другого агента.
...
- История обновляется через `session.events` + последовательные `/events?after_id=...` до пустой страницы/стабильного cursor; агент не добавил несуществующий `GET /chat/threads` или `GET /chat/history`.
