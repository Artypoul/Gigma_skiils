...
- В текущем ERP нет отдельных permission names `view-payments`, `edit-payments`, `view-subscriptions`, `edit-subscriptions`. Не выдумывай их в заявке.
- Billing webhook/admin действия остаются owner/admin role-gated, а не обычным permission name. Для них не обещай "готовый billing permission set" без отдельного решения владельца.
- Для обычных MCP tool'ов запрашивать только бизнес-права под реальные endpoint'ы. Начинать с read-only набора вроде `view-orders`, `view-nomenclatures`, а write-права добавлять отдельным решением.
- Если точные permission names неизвестны и human token сейчас недоступен, не выдумывай их: попроси owner/user назвать нужные read-only права или стартуй с минимального уже подтверждённого списка.
...
1. Уточнить у владельца или пользователя:
   ...
   - минимальный список `permissions` под конкретные MCP tool'ы;
...
1. Собрать минимальные входные данные:
   ...
   - минимальные `permissions`, например `["view-orders"]` только если они реально нужны и уже подтверждены;
