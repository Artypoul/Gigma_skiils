...
- Codex Desktop: `js_repl` is usually disabled there (`features.js_repl = false`) and cannot be toggled mid-session. Do NOT block the task on this precondition — instead check the actual tool inventory of the running session and route to the first available option: (1) an in-app browser capability if the session really exposes one (Browser plugin / `node_repl` browser API), otherwise (2) the bundled `$playwright` terminal workflow, which needs no `js_repl`. For an Electron target this fallback does not apply: the in-app browser and the bundled `$playwright` workflow drive real browser URLs only, while the Electron path here needs `_electron.launch` and main-process access. Without `js_repl` (or another Electron-capable tool) report that limitation for Electron tasks instead of substituting a browser-only route. Only if no suitable capability exists, report the limitation instead of silently stopping. State the chosen route once in the report. **The whole workflow below is written for `js_repl` sessions only** — it imports Playwright into a persistent kernel and reuses `js_repl` bindings. Do not apply it to an alternate capability: drive an in-app browser through that tool's own documented interface, and use `$playwright` through its own terminal workflow. Keep only the QA inventory and the visual/functional checklists as shared discipline.
