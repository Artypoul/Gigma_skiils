...
Для текущего Gigma AI miniapp product decision: статический source token принят как временное продуктовое решение. Не ротируй, не заменяй, не переносишь его в другую схему auth и не называй это blocker'ом без прямой команды Art. Проверяй только, что frontend/runtime передаёт существующее значение как `X-Source-Secret`, а backend принимает его для `source=miniapp`, `project_external_ref=gigma-ai`, `app_external_ref=gigma-ai`.
...
- Для Gigma AI miniapp статический source token — зафиксированное продуктовое решение. Если ревью или агент ругается на static token, отвечай: это осознанный временный продуктовый компромисс; проверяем blast radius и передачу через `X-Source-Secret`, но не ротируем и не меняем auth-flow без прямой команды Art.
