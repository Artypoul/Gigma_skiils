...
from evidence_integrity import EvidenceError, candidate_closure, command_executable_records_for_specs, load_json, sha256_file, verify_contract
...
def responsive_matrix(widths: list[int], heights: dict[str, int], breakpoints: list[int]) -> list[dict]:
    ...
    if len(decoded) > 1:
        if not boundaries:
            raise EvidenceError("multiple decoded widths require complete generated donor breakpoints")
...
def main() -> int:
    ...
    parser.add_argument("--breakpoints", default="", help="Optional assertion only: must exactly match breakpoints generated by the pinned donor classification")
    ...
    breakpoints = sorted(set(int(value) for value in (bundle.get("classification", {}).get("breakpoints") or [])))
    supplied_breakpoints = sorted(set(int(value) for value in args.breakpoints.split(",") if value.strip()))
    if supplied_breakpoints and supplied_breakpoints != breakpoints:
        raise EvidenceError("--breakpoints differs from the generated donor breakpoint source")
    ...
    lifecycle_cwd = (candidate_root / (lifecycle.get("cwd") or ".")).resolve() if lifecycle else candidate_root
    lifecycle_cwd.relative_to(candidate_root)
    command_specs = [(command, candidate_root) for command in commands]
    for key in ("build", "start", "teardown"):
        if lifecycle and lifecycle.get(key):
            command_specs.append((lifecycle[key], lifecycle_cwd))
    executable_records = command_executable_records_for_specs(command_specs)
    contract = {
        ...
        "breakpoint_source": {"kind": "generated-reference-classification", "donor_identity": bundle.get("donor_identity"), "breakpoints": breakpoints},
        "candidate": {
        ...
        "command_executables": executable_records,
