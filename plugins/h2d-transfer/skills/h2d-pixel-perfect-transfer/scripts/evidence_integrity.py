...
def verify_contract(contract_path: Path, evidence_dir: Path | None = None) -> dict[str, Any]:
    ...
    for command in command_sets:
        if not isinstance(command, list):
            raise EvidenceError("all pinned commands must be string arrays")
        for value in command[1:]:
            if not isinstance(value, str) or Path(value).suffix.lower() not in {".py", ".js", ".mjs", ".cjs", ".sh", ".ps1", ".cmd", ".bat"}:
                continue
            path = Path(value)
            path = path.resolve() if path.is_absolute() else (root / path).resolve()
            if not path.is_file():
                raise EvidenceError(f"pinned command input is missing: {path}")
            if path not in candidate_files and path not in sidecar_files:
...
def verify_current_evidence(evidence_path: Path) -> dict[str, Any]:
    ...
    verify_hashed_files(evidence_path.parents[1], evidence.get("reports") or [], "current reports")
