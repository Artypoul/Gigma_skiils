    ...
    for trace_id in sorted(expected_ids):
        ...
        if not expected or not actual:
            ...
            for index, expected_sample in enumerate(expected_samples):
                ...
                passed = not style_diff and ratio <= args.max_frame_mismatch_ratio and canvas_ok
                frame_results.append({"index": index, "t_ms": expected_sample.get("t_ms"), "result": "pass" if passed else "fail", "pixel_mismatch_ratio": ratio, "style_diff": style_diff, "canvas_match": canvas_ok})
