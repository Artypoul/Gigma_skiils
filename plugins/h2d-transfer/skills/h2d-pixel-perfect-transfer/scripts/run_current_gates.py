...
def verify_matrix_artifacts(output: Path, coverage: dict, classification: dict, expected_matrix: list[str], commands: list[list[str]] | None = None, candidate_root: Path | None = None) -> None:
    ...
    for role, (relative, allowed_verdicts) in role_paths.items():
        ...
        if report.get("generator_sha256") != sha256_file(specialist) or not commands or candidate_root is None or not command_invokes_script(commands, specialist, candidate_root):
...
def build_matrix_coverage(output: Path, classification: dict, expected_matrix: list[str], commands: list[list[str]], candidate_root: Path) -> dict:
    artifacts = []
    ...
    verify_matrix_artifacts(output, coverage, classification, expected_matrix, commands, candidate_root)
    return coverage
...
def main() -> int:
    ...
    coverage = build_matrix_coverage(output, contract.get("classification") or {}, expected_matrix, commands, candidate_root)
