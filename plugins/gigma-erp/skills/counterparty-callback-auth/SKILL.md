...
Если у приложения есть свой сервер, BFF или SSR-контур, предпочитать server-wrapper: браузер общается со своими endpoint'ами, а не держит сырой `session_token`.
...
- ваш сервер -> `POST /api/counterparty/callback_auth/init`
- ваш сервер сохраняет `session_token` только в `httpOnly` cookie или серверной сессии
- браузер получает только `callback_number`, `expires_at` и локальный UI-state
- браузер -> ваш `POST /login/callback-auth/status`
- ваш сервер читает `session_token`, ходит в Gigma `status`, при `verified` ставит финальную auth cookie и очищает callback-session
...
- браузер сам вызывает Gigma `init` и `status` с App Token
- `session_token` живёт только в памяти текущей вкладки
- после `verified` frontend сохраняет только итоговый `access_token`
...
- Не класть `session_token` в `localStorage`, `sessionStorage`, URL, query params или non-`httpOnly` cookie, если у приложения есть свой серверный слой.
- Не возвращать сырой `session_token` из своих публичных app-endpoint'ов, если можно сохранить его серверно и поллить через local endpoint.
...
- В server-wrapper режиме polling должен идти через ваш local endpoint без передачи raw `session_token` из браузера.
- В direct-storefront режиме `session_token` допустим только в оперативной памяти вкладки; после reload или закрытия вкладки его не восстанавливать из browser storage.
