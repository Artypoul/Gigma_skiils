...
`permissions` в заявке — это обычные ERP permission names из таблицы `permissions`
с `guard_name=user`, а не отдельный MCP-набор.

Runtime source of truth:

- `GET /api/permissions` с human Bearer token.

Supported zero-token fallback:

- `agent-permission-profiles.md` в этом plugin reference.

Если human token уже есть, `GET /api/permissions` важнее reference-файла. Если
human token нет, агент может безопасно выбрать один из готовых presets из
`agent-permission-profiles.md` вместо требования "назовите exact permission names".
...
### Zero-token permission presets

Для типовых self-service заявок используй готовые профили из
`agent-permission-profiles.md`:

- `read_only_assistant`
- `business_assistant`
- `trusted_operator`

Фраза "максимальные разумные полномочия помощника" по умолчанию означает
`business_assistant`: широкий business-access без `edit-admins`,
`edit-permissions`, `manage-agent-tokens` и других system rights.
