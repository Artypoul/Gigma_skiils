...
| Auth/payment/subscription/webhook/user flow | `project-context-bootstrap` -> `ddd-api-standard` when the change touches API contracts, invariants, access rights, money, limits or the data schema -> local flow/data review skills -> `feature` if implementation is needed |
| Backend/API design or review: domain model, bounded contexts, aggregates, transactions, concurrency, idempotency, migrations, events/outbox | `project-context-bootstrap` -> `ddd-api-standard` -> `feature` if implementation is needed |
