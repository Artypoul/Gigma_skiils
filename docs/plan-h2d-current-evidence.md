...
| Проверяем поведение рекурсивными безопасными последовательностями | Escape/outside-click запускались после reload; controls из открытого modal/submenu и listener-backed `div`/SVG/canvas не обнаруживались; формы/keyboard flow не проверялись | Matrix использует pre-load listener instrumentation, рекурсивно переинвентаризирует reached states и содержит prerequisites, input/select/validation/intercepted-submit, scroll/touch, Tab/Shift-Tab и role-specific keyboard sequences; каждый post-state имеет semantic/visual/accessibility comparison | По умолчанию original traversal идёт по offline snapshot; live mode требует exact request/action allowlist, блокирует WebSocket/download/protocol navigation и любые неразрешённые GET/other requests; reference action error всегда non-final, а no-op запрещён только когда sequence объявляет `expected_transition` | БД не меняется | behavior inventory/matrix/capture/compare |
...
БД не меняется. Предпочтительный источник behavior/liveness — offline runnable snapshot. Для live donor browser gate полностью блокирует WebSocket/EventSource, download, form submission, protocol navigation и любой запрос, не совпавший с exact method+canonical URL allowlist; отдельный safe-action allowlist ограничивает UI actions. Даже same-origin GET не выполняется без явного правила. Evidence редактирует userinfo, tokens, signatures, session ids и sensitive query/path fragments; сохраняются безопасный origin/path pattern и SHA-256 полной canonical URL. Единственные записываемые контракты — локальные JSON schemas, CLI и структура `.h2d`; они проверяются реальным donor-файлом и синтетическими fixtures без приватных данных.
...
- Reachable states: reference classification рекурсивно повторяет behavior/liveness/listener discovery после каждой безопасной sequence; canonical state fingerprint, visited-set, depth/branch budgets и cycle detection гарантируют завершение. Достигнутый budget даёт explicit owner-scoped incomplete-coverage decision и non-pass до сужения scope/allowlist.
- Listener surfaces: actual `addEventListener`, inline handlers и listener-backed plain DOM/SVG/canvas surfaces собираются pre-load instrumentation и входят в recursive matrix.
...
- Safe traversal: live requests и actions должны совпасть с exact allowlists; WebSocket/EventSource/navigation/write/download блокируются. Форма проверяется через typed input, select, validation и локально intercepted submission outcome без внешней записи.
...
8. Safe traversal/form fixture: unallowlisted GET, write request, WebSocket/EventSource, navigation/download блокируются; input/select/validation/intercepted submit проверяются без внешней записи; both-sides-error reference падает, legitimate hover/focus no-op без `expected_transition` проходит.
...
12. Selector/listener fixture: donor без id/markers и listener-backed plain div/SVG/canvas получает уникальные resolvable selectors; unresolved, non-unique или many-to-one mapping нельзя заморозить без approved equivalence.
...
19. Semantic/accessibility fixture: donor text/link не может заменить pinned candidate meaning; role/hierarchy/state regression и сломанный Tab/focus-trap падают без field-level approval.
...
21. Discovery-limit/cycle fixture: >200 controls, surface после 1500-го node, carousel/calendar cycles и exhausted traversal budget либо полностью учтены, либо дают explicit non-pass без hang.
22. Candidate-closure fixture: evidence output не меняет digest, а изменение любого included HTML/CSS/config/dependency/build input делает run stale.
23. Legacy bypass fixture: direct `run_all_gates.py` после candidate mutation не проходит без regeneration/current hashes.
24. Existing H2D 0.3.0 compatibility tests/fixtures для typography, selector-map, asset provenance и changed-source.
25. `npm run check:js`, package self-check, `quick_validate.py` для изменённых skills.
26. `bash sync-codex.sh`, `python validate_plugin.py` и тот же H2D suite в GitHub Actions.
